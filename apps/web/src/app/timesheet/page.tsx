...
export default function TimesheetPage() {
  ...
  const fetchWorklogs = useCallback(async () => {
    ...
    try {
      ...
      const worklogsRes = await fetch(apiUrl(`/api/tempo/worklogs?date=${dateStr}`));
  ...
  const suggestAll = async () => {
    ...
    try {
      const response = await fetch(apiUrl('/api/llm/suggest'), {
        method: 'POST',
  ...
  const handleLog = async (activityId: string, ticketKey: string) => {
    ...
    try {
      ...
      const overlapRes = await fetch(apiUrl('/api/tempo/check-overlap'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          date: dateStr,
