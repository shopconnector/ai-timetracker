...
export default function SettingsPage() {
  ...
  const [badSuggestions, setBadSuggestions] = useState<
    Array<{
      pattern: string;
  ...
  const [newTimeOffType, setNewTimeOffType] = useState<'vacation' | 'sick' | 'remote' | 'other'>(
    'vacation'
  );
  ...
  const loadAllData = useCallback(async () => {
    ...
    try {
      const res = await fetch('/timetracker/api/status');
    ...
    try {
      const settingsRes = await fetch('/timetracker/api/settings');
      if (settingsRes.ok) {
        ...
        setApiConfig({
          ...
          googleCalendarIcalUrl: settings.googleCalendarIcalUrl || '',
        });
      }
    } catch (error) {
  ...
  const handleSaveApiConfig = async () => {
    ...
    try {
      const res = await fetch('/timetracker/api/settings', {
        method: 'PUT',
  ...
  const loadJiraData = async () => {
    ...
    try {
      const [projectsRes, issuesRes] = await Promise.all([
        fetch('/timetracker/api/jira/projects'),
        fetch('/timetracker/api/jira/my-issues'),
      ]);
  ...
  const handleSearchIssues = async () => {
    ...
    try {
      const res = await fetch(
        `/timetracker/api/jira/my-issues?query=${encodeURIComponent(searchQuery)}`
      );
      if (res.ok) {
        const data = await res.json();
        setJiraIssues(data.issues || []);
    ...
    <div className="max-w-4xl space-y-6">
      ...
      <div>
        ...
        <p className="text-slate-500 dark:text-slate-400">
          Configure your TimeTracker integrations
        </p>
      </div>
          ...
          <div className="space-y-3">
            {apiStatus.map((api, index) => (
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  {api.message && (
                    <span className="max-w-xs truncate text-sm text-gray-500">{api.message}</span>
                  )}
            ...
            {apiStatus.length === 0 && !loadingStatus && (
              <p className="py-4 text-center text-gray-500">Brak danych o statusie API</p>
            )}
          </div>
        </CardContent>
      </Card>
          ...
          {targets && (
            <>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="mb-1 block text-sm text-gray-500">Cel dzienny (godziny)</label>
                  <Input
                    type="number"
                    min="1"
                    max="24"
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={e =>
                      handleSaveTargets({ weeklyHours: parseInt(e.target.value) || 40 })
                    }
                  />
                </div>
              </div>
              <div>
                <label className="mb-2 block text-sm text-gray-500">Dni robocze</label>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-3 rounded-lg bg-gray-50 p-3">
            <p className="text-sm font-medium">Dodaj urlop / wolne</p>
            <div className="grid grid-cols-4 gap-2">
              ...
              <Select
                ...
                onValueChange={(v: 'vacation' | 'sick' | 'remote' | 'other') =>
                  setNewTimeOffType(v)
                }
              >
                <SelectTrigger>
          ...
          {timeOffs.length > 0 && (
            <div className="space-y-2">
              <p className="text-sm font-medium text-gray-600">Zaplanowane wolne:</p>
              {timeOffs.map(to => (
                <div
                  key={to.id}
          ...
          {/* Add Holiday */}
          <div className="space-y-3 rounded-lg bg-gray-50 p-3">
            <p className="text-sm font-medium">Dodaj święto / dzień wolny</p>
          ...
          {/* Holidays List */}
          <div className="space-y-2">
            <p className="text-sm font-medium text-gray-600">Święta w tym roku:</p>
            <div className="flex flex-wrap gap-2">
              {holidays.slice(0, 15).map((h, i) => (
                <Badge key={i} variant="outline" className="text-xs">
                  {h.isRecurring ? '🔄' : '📅'} {h.date.length === 5 ? h.date : h.date.slice(5)} -{' '}
                  {h.name}
                  {!h.isRecurring && (
                    <button
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <CardDescription>
                  Klucze API do Tempo, Jira, ActivityWatch i OpenRouter
                </CardDescription>
              </div>
            </div>
            <div className="flex gap-2">
              <Button variant="outline" size="sm" onClick={handleTestApis} disabled={testingApis}>
                <TestTube className={`mr-2 h-4 w-4 ${testingApis ? 'animate-pulse' : ''}`} />
                Testuj
              </Button>
              <Button size="sm" onClick={handleSaveApiConfig} disabled={savingConfig}>
                <Save className={`mr-2 h-4 w-4 ${savingConfig ? 'animate-spin' : ''}`} />
                Zapisz
              </Button>
            </div>
          </div>
        ...
        <CardContent className="space-y-6">
          ...
          {Object.keys(testResults).length > 0 && (
            <div className="space-y-2 rounded-lg bg-gray-50 p-4">
              <p className="mb-2 text-sm font-medium">Wyniki testów:</p>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="mb-1 block text-sm text-gray-500">API Token</label>
                <Input
                  type="password"
              ...
              <div>
                <label className="mb-1 block text-sm text-gray-500">Account ID</label>
                <Input
                ...
                />
                <p className="mt-1 text-xs text-gray-400">Twoje Atlassian Account ID</p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-3 gap-4">
              <div>
                <label className="mb-1 block text-sm text-gray-500">Base URL</label>
                <Input
          ...
          <div className="space-y-3">
            ...
            <div>
              <label className="mb-1 block text-sm text-gray-500">Server URL</label>
              <Input
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="mb-1 block text-sm text-gray-500">API Key</label>
                <Input
                  type="password"
                ...
                />
                <p className="mt-1 text-xs text-gray-400">Pobierz z: openrouter.ai/keys</p>
              </div>
              <div>
                <label className="mb-1 block text-sm text-gray-500">Model LLM</label>
                <Select
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="openai/gpt-4o-mini">GPT-4o Mini (tani, $0.15/1k)</SelectItem>
          ...
          <div className="space-y-3">
            ...
            <div>
              <label className="mb-1 block text-sm text-gray-500">iCal URL (prywatny link)</label>
              <Input
                type="url"
                placeholder="https://calendar.google.com/calendar/ical/..."
                value={apiConfig.googleCalendarIcalUrl}
                onChange={e =>
                  setApiConfig({ ...apiConfig, googleCalendarIcalUrl: e.target.value })
                }
              />
              <p className="mt-1 text-xs text-gray-400">
                Jak uzyskać: Google Calendar → Ustawienia → Twój kalendarz → Integruj kalendarz →
                Tajny adres w formacie iCal
              </p>
              <div className="mt-2 rounded bg-yellow-50 p-2 text-xs text-yellow-800">
                ...
                <ol className="ml-4 mt-1 list-decimal space-y-1">
                  ...
                  <li>Kliknij na swój kalendarz po lewej stronie</li>
                  <li>Przewiń do sekcji &quot;Integruj kalendarz&quot;</li>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex items-end gap-2">
            <div className="flex-1">
              <label className="mb-1 block text-sm text-gray-500">Nazwa projektu</label>
              <Input
            ...
            <div className="flex-1">
              <label className="mb-1 block text-sm text-gray-500">Task Jira</label>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <CardDescription>Statystyki sugestii i uczenie się z feedbacku</CardDescription>
              </div>
            </div>
                      ...
                      <Badge key={source} variant="outline" className="text-xs">
                        {source === 'llm'
                          ...
                          : source === 'history'
                            ? '📚 Historia'
                            : '📁 Mapping'}
                        :<span className="ml-1 text-green-600">+{stats.positive}</span>
                        <span className="ml-1 text-red-600">-{stats.negative}</span>
                      </Badge>
                    ))}
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-1">
                    {badSuggestions.slice(0, 5).map((bad, i) => (
                      ...
                      >
                        <span className="flex-1 truncate" title={bad.pattern}>
                          {bad.pattern.split(':')[0]} →{' '}
                          <span className="font-mono">{bad.suggestedTicket}</span>
                        </span>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <CardDescription>Szczegółowa historia decyzji AI vs użytkownik</CardDescription>
              </div>
            </div>
            <div className="flex gap-2">
            ...
            <>
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-3 gap-3">
                  {Object.entries(auditStats.bySource).map(([source, data]) => (
                    <div key={source} className="rounded border p-2 text-center">
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-4">
                  ...
                  <div className="flex-1 rounded bg-gray-50 p-2">
                    ...
                    <div className="font-bold">
                      {Math.round((auditStats.recentTrend.last30days.accuracy || 0) * 100)}%
                      trafność
                    </div>
                    <div className="text-xs text-gray-400">
              ...
              {auditStats.topRejectedSuggestions.length > 0 && (
                <div className="space-y-2">
                  ...
                  <div className="space-y-1">
                    {auditStats.topRejectedSuggestions
                      .slice(0, 3)
                      .map(({ suggested, actual, count }, i) => (
                        <div
                          key={i}
                          className="flex justify-between rounded bg-yellow-50 p-2 text-xs"
                        >
                          <span>
                            <span className="text-gray-400 line-through">{suggested}</span>
                            {' → '}
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>{taskHistory.length} tasków w historii</CardDescription>
            </div>
            ...
            {taskHistory.slice(0, 20).map(task => (
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="max-w-xs truncate text-sm text-gray-500">{task.name}</span>
                </div>
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>{jiraProjects.length} projektów dostępnych</CardDescription>
            </div>
            <Button variant="outline" size="sm" onClick={loadJiraData} disabled={loadingJira}>
              <RefreshCw className={`mr-2 h-4 w-4 ${loadingJira ? 'animate-spin' : ''}`} />
              Odśwież
            </Button>
          </div>
        </CardHeader>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Eksportuj lub importuj historię i mapowania</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex gap-4">
