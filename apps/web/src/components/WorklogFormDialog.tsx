...
export function WorklogFormDialog({
  ...
  useEffect(() => {
    const fetchAttributes = async () => {
      setLoading(true);
      try {
        const res = await fetch(apiUrl('/api/tempo/attributes'));
  ...
  useEffect(() => {
    const checkOverlap = async () => {
      ...
      try {
        const res = await fetch(apiUrl('/api/tempo/check-overlap'), {
          method: 'POST',
  ...
  const handleAutoFill = async () => {
    ...
    try {
      ...
      const res = await fetch(apiUrl('/api/llm/suggest-worklog'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          activity: {
